...
--[[
    if ActivityExtendGift.CanShowIcon(actor) then
        ...
        buttonstr = buttonstr..'<Button|id='..MAINICON_ID_1..'|x='..currIconX..'|y=35|nimg=private/cc_func_icon/2.png|link=@topicon_openpanel#sid='..ICON_EXTENDGIFT..'>'        
    end
    if ActivityOpenServer.CanShowIcon(actor) then
        currIconX = currIconX - 80
        buttonstr = buttonstr..'<Button|id='..MAINICON_ID_2..'|x='..currIconX..'|y=35|nimg=private/cc_func_icon/1.png|link=@topicon_openpanel#sid='..ICON_OPENSERVERACTIVITY..'>'
    end   
]]--
    if ActivityNewPlayerRecharge.CanShowIcon(actor) then        
        currIconX = currIconX - 80
        buttonstr = buttonstr..'<Button|id='..MAINICON_ID_3..'|x='..currIconX..'|y=10|nimg=private/cc_func_icon/3.png|link=@topicon_openpanel#sid='..ICON_NEWPLAYER_RECHARGEACTIVITY..'>'
    end        
...
--[[
    ...
    if curlv >= 40 then
        ...
        local buttonstr1 = ''
        currIconX = -280
        buttonstr1 = buttonstr1..'<Button|x='..currIconX..'|y=75|nimg=custom/a00002.png|width=70|height=70|link=@元宝充值>'
        currIconX = currIconX - 80
        buttonstr1 = buttonstr1..'<Button|x='..currIconX..'|y=75|nimg=custom/a00016.png|width=70|height=70|link=@唯一会员>'
        addbutton(actor, 102, CommonDefine.ADD_BUTTON_ID_34, buttonstr1)
    end
end
