...
function ItemTips.CreateEquipPanel(data, itemData, isWear, panelInsertIndex)
    ...
    if isWear ~= false and data and data.from == ItemFrom.BAG then
        if data.issuperbox~=nil and data.issuperbox==true then
            ItemTips.AddButton(ListBg, itemData, 1)
            ItemTips.AddButton(ListBg, itemData, 4)
...
local _GetBtnCfg = function(btnType)
    local cfg = {
        ...
        [4] = {
            ...
            btnName     = "保留", 
            func        = function(data)
                if data and data.MakeIndex then
                    SL:SendNetMsg(CCMsgDefine.CM_BOX_EQUIPITEM_COMPARE_OPER, data.MakeIndex, 0, 0, '')
                end
                SL:CloseItemTips()
            end
        },      
...
--btnType: 1： 佩戴    2：拆分    3：使用道具   4：宝箱保留
function ItemTips.AddButton(parent, itemData, btnType)
    ...
    if btnType == 4 then
        GUI:setPositionX(button, posX + 80)
    end
